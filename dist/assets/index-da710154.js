 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const Ng=5*60,Mg=yl("authIdTokenMaxAge")||Ng;let zo=null;const Dg=t=>async e=>{const n=e&&await e.getIdTokenResult(),s=n&&(new Date().getTime()-Date.parse(n.issuedAtTime))/1e3;if(s&&s>Mg)return;const r=n==null?void 0:n.token;zo!==r&&(zo=r,await fetch(t,{method:r?"POST":"DELETE",headers:r?{Authorization:`Bearer ${r}`}:{}}))};function yn(t=Eh()){const e=xl(t,"auth");if(e.isInitialized())return e.getImmediate();const n=Mh(t,{popupRedirectResolver:Pg,persistence:[Fp,Cp,Yl]}),s=yl("authTokenSyncURL");if(s){const i=Dg(s);Ep(n,i,()=>i(n.currentUser)),wp(n,o=>i(o))}const r=ud("auth");return r&&sp(n,`http://${r}`),n}Og("Browser");const ce=sd("store",{state:()=>({bg:"bg-white",modal:!1,feedback:!1,comp:"Contacts",loggedIn:!1,uid:"",name:"",email:"",data:{},dataOpt:{},cart:{},lang:"ru",langProp:{}}),getters:{},actions:{sum(){let t=0;for(let e in this.cart)console.log(this.cart[e].price),t+=this.cart[e].price*this.cart[e].quantity;return t},async process(){let t=await fetch("/api/language",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({1:this.lang})});this.langProp=await t.json()},async checkStatus(){return new Promise((t,e)=>{const n=yn();Ip(n,s=>{s&&(console.log("works"),this.loggedIn=!0,this.uid=s.uid,this.name=s.displayName,this.email=s.email,console.log(s)),t(s)})})},async getData(){try{let t=await fetch("/api/productsList");this.data=await t.json(),t=await fetch("/api/productsListOpt"),this.dataOpt=await t.json()}catch(t){console.log(t)}},async getCart(){try{let e=await(await fetch("/api/usersCart",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({1:this.uid})})).json();console.log(e),e=JSON.parse(e.orders),console.log(e);for(let n in e){console.log(n);let s;try{s=JSON.parse(JSON.stringify(this.data[e[n].id])),s.quantity=e[n].quantity,console.log(s),this.cart[s.id]=s}catch(r){console.log(r),s=JSON.parse(JSON.stringify(this.dataOpt[e[n].id])),s.quantity=e[n].quantity,this.cart[s.id]=s}}}catch(t){localStorage.getItem("cart"==null)&&(this.cart=JSON.parse(localStorage.getItem("cart"))),console.log(t)}}}});var Lg="firebase",Ug="9.17.1";/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");

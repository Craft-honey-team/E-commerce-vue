<template>
   ...
   <header class="bg-white flex justify-between my-[10px] max-[800px]:grid-cols-1">
		...
		<img @click="burgerMenu = true" class = "justify-self-center self-center hover:cursor-pointer" src = "../assets/menu.svg">
	</div>
		...
		<div class = "flex items-center mr-[100px] max-[1300px]:mr-[40px]">
			...
			<button class = "text-xl text-center" @click = "store.modal = true, store.comp = 'Login', store.bg = 'bg-bee'">Вход</button>
		</div>
	...
	<div class="right-[0px] top-0 overflow-hidden transition-all duration-500 ease-in-out z-50 absolute h-screen bg-[#DAE2E2]" :class="burgerMenu ? 'w-[330px]' : 'w-[0px]'">
		<div class=" ">
			<div class="flex flex-col justify-center [&>*]:px-[10px]">
				<button @click="burgerMenu = false" class="self-end w-fit focus:outline-none mr-2 mt-2 text-[22px] text-right">
					<strong>X</strong> 
				</button>

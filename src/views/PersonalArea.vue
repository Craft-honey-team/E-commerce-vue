	...
	<Layout>
		<div>
			...
			<div class="grid grid-flow-row gap-20">
				<div class="grid grid-flow-col auto-cols-max text-[20px] max-[700px]:grid-col-1">
					<span
				...
				<div class="grid grid-flow-col gap-[30px] text-[20px]">
					<div class="text-slate-500">
						<p class=" ">{{ store.langProp.yourname }}:</p>
						<p class=" ">Email:</p>
						<p class=" ">{{ store.langProp.telephone }}</p>
						<p class=" ">{{ store.langProp.delivery }}:</p>
						<p class="">Способ доставки: {{ }}</p>
					</div>
					<div>
						<p class=" ">{{ store.name }}</p>
						<p class=" ">{{ store.email }}</p>
						<div v-show="setPhone">
							<input @input="handleInput" v-model="store.number" placeholder="Введите номер телефона"
								class="border-solid border-2 border-grey-600 rounded-[10px] px-[5px] w-[190px] text-[14px] mr-[20px]">
							<button @click="setPhone = false, savePhone(phone)"
								class="text-blue-700 underline decoration-1 text-[18px] hover:text-amber-700 hover:underline underline-offset-4">Добавить</button>
						</div>
						<div v-show="!setPhone">
							<span v-if="store.number.length" class="mr-[20px]">{{ store.number }}</span>
							<button @click="setPhone = true"
								class="text-blue-700 underline decoration-1 text-[18px] hover:text-amber-700 hover:underline underline-offset-4">Изменить
								номер</button>
						</div>
						<div v-show="setAddress">
							<input @input="handleInputAddress" v-model="store.address" placeholder="Введите адрес"
								class="border-solid border-2 border-grey-600 rounded-[10px] px-[5px] w-[190px] text-[14px] mr-[20px]">
							<button @click="setAddress = false"
								class="text-blue-700 underline decoration-1 text-[18px] hover:text-amber-700 hover:underline underline-offset-4">Добавить</button>
						</div>
						<div v-show="!setAddress">
							<span v-if="store.address.length" class="mr-[20px]">{{ store.address }}</span>
							<button @click="setAddress = true"
								class="text-blue-700 underline decoration-1 text-[18px] hover:text-amber-700 hover:underline underline-offset-4">Изменить
								адрес</button>
						</div>
						<div class="mt-[-15px]">
					...
					<div class="ml-[50px] gap-[10px] leading-4">
						...
						<div class="grid grid-flow-col gap-[10px]">
							<img @click="dhl = !dhl" class="w-[100px] h-[80px] hover:cursor-pointer"
								src="@/assets/dhl-express.svg" alt="">
							<img @click="dpd = !dpd" class="w-[100px] h-[80px] hover:cursor-pointer"
								src="@/assets/dpd-logo-2015-.svg" alt="">
							<img @click="cdek = !cdek" class="w-[100px] h-[80px] hover:cursor-pointer"
				...
				<button @click="resetPassword"
					class="text-blue-700 underline decoration-1 text-[18px] hover:text-amber-700 hover:underline underline-offset-4"
					to="">Изменить пароль</button>
				<button @click="exitPersonalArea"
					class="bg-[#EAAD02] h-[30px] w-[200px] rounded-[10px] px-2 text-center text-white  hover:text-amber-700 hover:underline underline-offset-4">Выйти
					из АКК</button>
			</div>
		</div>
	</Layout>

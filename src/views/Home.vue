  ...
  <div class="h-full justify-center flex items-center">
    <div class = "grid gap-[10vh] [&_*]:text-white">
      ...
      <div class="justify-center flex max-[800px]:pt-[10px]">
		...
		<router-link to="/Blog" class="relative z-10  flex items-center justify-center bg-yellow-500 w-40 h-12 rounded-md">{{ store.langProp[store.lang].blog }}</router-link>          	
          	<button @click = "store.modal = true, store.comp = 'Contacts', store.bg = 'bg-white'" class="relative z-10 bg-yellow-500 w-40 h-12 rounded-md">{{ store.langProp[store.lang].contacts }}</button>
          </div>
      </div>
    </div>
  </div>
